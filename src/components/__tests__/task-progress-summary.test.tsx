...
describe('TaskProgressSummary — 集計', () => {
  it('完了 ToDo 数 / 全 ToDo 数 を「完了 X/N」形式で表示', () => {
    const html = renderToStaticMarkup(
      <TaskProgressSummary scheduledPct={50} completed={1} total={5} />,
    )
  ...
  it('全完了 → 100%', () => {
    const html = renderToStaticMarkup(
      <TaskProgressSummary scheduledPct={50} completed={5} total={5} />,
    )
  ...
  it('全未着手 → 0%', () => {
    const html = renderToStaticMarkup(
      <TaskProgressSummary scheduledPct={50} completed={0} total={5} />,
    )
  ...
  it('ToDo が 0 件のエッジケースは 0% / 0/0 表示 (ゼロ除算回避)', () => {
    const html = renderToStaticMarkup(
      <TaskProgressSummary scheduledPct={50} completed={0} total={0} />,
    )
    expect(html).toContain('0%')
...
describe('TaskProgressSummary — 色付け', () => {
  ...
  it('部分完了 → 中間色 (緑ではない)', () => {
    const html = renderToStaticMarkup(
      <TaskProgressSummary scheduledPct={50} completed={2} total={5} />,
    )
